...
配置项可以通过多种方式传递给 `babel`，主要取决于[如何使用 babel](/babel/#babel-的使用方式)，
...
这些选项只允许作为[ Babel 编程方式](/babel/#babel-core-编程的方式)传递，直接传递给 `babel`。
...
配置位置：仅在[ Babel 的编程选项]((/babel/#babel-core-编程的方式))中允许
...
1. `targets` 选项是用来确定目标环境的，查看[`babel` 工具包解析 targets](https://babel.docschina.org/docs/en/babel-helper-compilation-targets/)，最终会输出一个目标对象：
...
* 可以是与[browserslist 兼容的](https://github.com/ai/browserslist)查询（带有[警告](https://babel.docschina.org/docs/en/options/#ineffective-browserslist-queries)）
...
* 或支持环境版本的对象：支持的环境：`android`, `chrome`, `edge`, `electron`, `firefox`, `ie`, `ios`, `node`, `opera`, `rhino`, `safari`, `samsung`。如果未指定次要版本，Babel 会将其解释为MAJOR.0. 例如，"node": 12将被视为 Node.js 12.0。
...
* 不指定值时(没有目标时)：Babel 会假设目标是最旧的浏览器(例如，@babel/preset-env将所有 ES2015-ES2020 代码转换为 ES5 兼容)
...
* `targets.esmodules`：`boolean`类型，当为 `true`，会假设环境为支持 `ES` 模块的浏览器。
  ...
  >
  >   * 插件/预设 配置项控制，例如：@babel/preset-env 通过 [`modules: false` ](https://babel.docschina.org/docs/en/babel-preset-env/#modules)
  >
  >     ```json
  >     "presets": [
  >       [
  ...
  >     ```
  >
  >   * Babel 的编程程序中通过 [`caller`](https://babel.docschina.org/docs/en/options/#caller) 配置项
...
允许用户提供一组选项，这些选项将一次[合并](https://babel.docschina.org/docs/en/options/#merging)到当前配置中。此功能最好与[`"test"`](https://babel.docschina.org/docs/en/options/#test)/ [`"include"`](https://babel.docschina.org/docs/en/options/#include)/[`"exclude"`](https://babel.docschina.org/docs/en/options/#exclude) 选项一起使用，以对特定条件执行不一样的配置。
...
* 类型：`MatchPattern | Array<MatchPattern>`( [`MatchPattern`](https://babel.docschina.org/docs/en/options/#matchpattern))
...
* 类型：( `Array<MatchPattern>`)[`MatchPattern`](https://babel.docschina.org/docs/en/options/#matchpattern)
* 配置位置：不允许在 `presets` 预设内
