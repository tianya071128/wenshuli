...
**例子**：表单某个文本输入框获得焦点后，表单会被设置颜色样式。

<css-test type=":focus-within" />
...
对于 `a` 链接，`:hover`、`:active`、`:link`、`:visited` 这几个伪类可能会相互覆盖影响(优先级问题)，最好遵循LVHA的先后顺序进行设置，即：`:link` — `:visited` — `:hover` — `:active`
...
**作用**：表示任何处于选中状态的**radio**(`<input type="radio">`), **checkbox** (`<input type="checkbox">`) 或("select") 元素中的**option** HTML元素("option")。
